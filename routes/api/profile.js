...
check('skills','Skills are required').not().isEmpty()]], async (req, res) => {
    ...
    const {
      ...
      //Build Social object. Set Social to empty object first to avoid reference error
      ...
      try{
        //Search for the profile with id(id is key of user table)
        let profile = await Profile.findOne({user: req.user.id});

        //If profile exists, update it and return latest data
        if(profile)
        {
            //Update 
        ...
        //profile doesn't exists, so create one
      ...
      {
          console.error(err.message);
          return res.status(500).send('Server Status');
      }
...
// @desc     Get user's profile from github
// @access   Public
